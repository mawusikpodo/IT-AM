  ...
  return (
    <Card>
      <CardHeader className="relative">
        ...
        <hr />
        <span className="text-xs text-gray-700">Assigned to</span>
        <p> {file.assignedTo}</p>
      ...
      {/* <CardContent className="h-[100px] flex justify-center items-center">
        {file.type === "image" && file.url && (
          <Image alt={file.name} width="20" height="10" src={file.url} />
        )}

        {file.type === "csv" && <GanttChartIcon className="w-10 h-10" />}
        {file.type === "pdf" && <FileTextIcon className="w-10 h-10" />}
      </CardContent> */}
      <span className="text-xs text-gray-700 m-7">Uploaded by</span>
